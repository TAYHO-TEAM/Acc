...
using SendMailAppointmentService.DB;
...
namespace SendMailAppointmentService.JobProccess
{
    public class MainProccess
    {
        private readonly ProfileMailOptions _profileMailOptions;
        public async Task RunProccess(ProjectManagerBaseContext dbContext)
        {
            JobPlanProccess newProccess = new JobPlanProccess(dbContext);
            var test =await newProccess.GetPlanMasterReport();
        }
        public void SendMailAppointment(DateTime StartDate, DateTime EndDate, string LocationTaget, string Subject, string Body, string DisplayName, string MailFrom, List<string> MailTo, List<string> MailCC, List<string> MailBCC, bool isCancel)
        {
