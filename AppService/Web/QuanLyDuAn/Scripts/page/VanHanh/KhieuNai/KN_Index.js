...
function newComplaint() {
    ...
    $('#elementCompaintDate').dxDateBox('instance').option('value', new Date());
...
$("#form-Complaint").on("submit", function (e) {
    ...
    if (!resValid) {
        ...
        if (!$issetCP) {
            formdata.status = 1;
            console.log($('#elementCompaintDate').dxDateBox('instance').option('value'));
            formdata['complaintDate'] = formdata['complaintDate'] ?? new Date();
            customStore().store().insert(formdata).done((rs) => {
                ...
                if (rs.isOk) {
                    ...
                    $('#schedule').dxButton('instance').option('text', 'Cập nhật');
