...
{
    public class CategoryUnitController : APIControllerBase
    {
        const string VALIDATION_ERROR = "The request failed due to a validation error";
        public string nameEF = OperationManagerConstants.CategoryUnit_TABLENAME;
        protected readonly IQuanLyVanHanhRepository<CategoryUnit> _quanLyVanHanhRepository;
        public CategoryUnitController(IMapper mapper, IHttpContextAccessor httpContextAccessor, IQuanLyVanHanhRepository<CategoryUnit> quanLyVanHanhRepository) : base(mapper, httpContextAccessor)
        {
        ...
        public async Task<IActionResult> Post([FromForm]string values)
      {
        ...
        public async Task<IActionResult> Put([FromForm] int key, [FromForm] string values)
        {
            try
            {
                CategoryUnit model = new CategoryUnit();
                if (!string.IsNullOrEmpty(values))
                    JsonConvert.PopulateObject(values, model);
                IFormFileCollection files = Request.Form.Files;
                model.Id = key;
