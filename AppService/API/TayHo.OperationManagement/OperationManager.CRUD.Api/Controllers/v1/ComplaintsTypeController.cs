...
{
    public class ComplaintsTypeController : APIControllerBase
    {
        const string VALIDATION_ERROR = "The request failed due to a validation error";
        public string nameEF = OperationManagerConstants.ComplaintsType_TABLENAME;
        protected readonly IQuanLyVanHanhRepository<ComplaintsType> _quanLyVanHanhRepository;
        public ComplaintsTypeController(IMapper mapper, IHttpContextAccessor httpContextAccessor, IQuanLyVanHanhRepository<ComplaintsType> quanLyVanHanhRepository) : base(mapper, httpContextAccessor)
        {
            _quanLyVanHanhRepository = quanLyVanHanhRepository;
        }
        /// <summary>
        /// Get List of ComplaintsType.
        /// </summary>
        ...
        public async Task<IActionResult> Post([FromForm]string values)
      {
        ...
        public async Task<IActionResult> Put([FromForm] int key, [FromForm] string values)
        {
            try
            {
                ComplaintsType model = new ComplaintsType();
                if (!string.IsNullOrEmpty(values))
                    JsonConvert.PopulateObject(values, model);
                IFormFileCollection files = Request.Form.Files;
                model.Id = key;
