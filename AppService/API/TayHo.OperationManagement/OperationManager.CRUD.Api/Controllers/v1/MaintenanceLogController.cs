...
{
    public class MaintenanceLogController : APIControllerBase
    {
        const string VALIDATION_ERROR = "The request failed due to a validation error";
        public string nameEF = OperationManagerConstants.MaintenanceLog_TABLENAME;
        protected readonly IQuanLyVanHanhRepository<MaintenanceLog> _quanLyVanHanhRepository;
        public MaintenanceLogController(IMapper mapper, IHttpContextAccessor httpContextAccessor, IQuanLyVanHanhRepository<MaintenanceLog> quanLyVanHanhRepository) : base(mapper, httpContextAccessor)
        {
            _quanLyVanHanhRepository = quanLyVanHanhRepository;
        }
        /// <summary>
        /// Get List of MaintenanceLog.
        /// </summary>
        ...
        public async Task<IActionResult> Post([FromForm]string values)
      {
        ...
        public async Task<IActionResult> Put([FromForm] int key, [FromForm] string values)
        {
            try
            {
                MaintenanceLog model = new MaintenanceLog();
                if (!string.IsNullOrEmpty(values))
                    JsonConvert.PopulateObject(values, model);
                IFormFileCollection files = Request.Form.Files;
                model.Id = key;
