...
namespace  ProjectManager.CMD.Api.Application.Commands
{
    public class CreateSysJobCommandHandler : SysJobCommandHandler, IRequestHandler<CreateSysJobCommand, MethodResult<CreateSysJobCommandResponse>>
    {
        ...
        public async Task<MethodResult<CreateSysJobCommandResponse>> Handle(CreateSysJobCommand request, CancellationToken cancellationToken)
        {
            ...
            var newSysJob = new SysJob(request.JobName,request.NameDataBase,request.NameStoreProce, request.ConnStringHash, request.StartTime,request.EndTime,request.StartDate,request.EndDate,request.FirstDate,request.LastDate,request.NextDate,request.Times,request.Unit,request.StepTime,request.IsTemplate);
